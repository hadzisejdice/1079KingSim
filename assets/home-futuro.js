...
(() => {
  ...
  (function parallaxAurora(){
    ...
    window.addEventListener('mousemove', e=>{
      ...
      raf = requestAnimationFrame(()=>{ aurora.style.transform = `translate(${x*12}px, ${y*8}px)`; });
    }, { passive:true });
  })();
  ...
  (function countUp(){
    ...
    if ('IntersectionObserver' in window && hero){
      const io = new IntersectionObserver(e=>{
        if (e[0].isIntersecting){ counters.forEach(run); io.disconnect(); }
      }, { threshold:.35 });
      io.observe(hero);
    } else { counters.forEach(run); }
  })();

  /* Click ripple (buttons, tabs, pills) – uses .ripple styles from theme-all.css */
  (function ripple(){
  ...
  /* Slab carousel + Module cards → switch panel */
  (function slabCarousel(){
    ...
    // Cards also update the slab on hover/focus
  ...
  /* Smooth scroll for the cue button */
  (function smoothCue(){
...
// Home: colorize "Add Troops" (header) + all feature "Open" links like Mystic Trials button
(() => {
  // 1) Header "Add Troops"
  const header = document.querySelector('.nav .nav-inner');
  if (header){
    const addTroops = [...header.querySelectorAll('a,button')].find(a => /add troops/i.test(a.textContent));
    if (addTroops){
      addTroops.classList.add('btn-primary--compact');
      if (addTroops.tagName !== 'A'){ // if someone left it as a button/span, make it a link
  ...
  // 2) Feature cards "Open" → gradient CTA
  ...
  featureOpens.forEach(a => {
    if (/^open\b/i.test(a.textContent.trim())) {
      ...
      // make it feel like a button line
...
// Sticky bottom "Scroll" hint: inject + smart behavior (orange)
(() => {
  // Inject once
  ...
  const updateState = () => {
    // Hide around hero area if you want: keep visible at top as a hint
    // Toggle to "Up" when near bottom; otherwise "Scroll" (down)
    if (nearBottom()){
    ...
    // Optionally fade when user is mid-page and actively scrolling fast: keep always visible here
  };

  // Click: smart scroll
  hint.style.pointerEvents = 'auto';     // now clickable
...
// Home: colorize Open buttons in the Troops/Magic/Option-A/Mystic box
// and the top Bear Magic / Bear Option-A buttons (red), shrink-wrap width (yellow).
(() => {
  // 1) "Open" links inside cards/panels
  ...
  // 2) The small top buttons: "Mystic Trials", "Bear Magic", "Bear Option-A"
  // These can be <a> or <button>. Add the compact primary look to Magic & Option-A (and keep Trials if you want uniformity).
  ...
  topButtons.forEach(el => {
    ...
    if (label === 'bear magic' || label === 'bear option‑a' || label === 'bear option-a' || label === 'mystic trials') {
      // If it's a button-like chip, upgrade its look to compact primary
      el.classList.add('btn-primary--compact');
      // remove old pill borders if present
      el.style.border = '0';
...
// Remove any legacy in-page Scroll button (green)
(() => {
